...
lastestFixDataVersionsCollection.forEach(function(eachLastestFixDataVersionsModel) {
    ...
    versionArray.forEach(function(eachRow) {
        if (eachRow["table"] == objectTableName) if (eachLastestFixDataVersionsModel.get("version") != eachRow["version"]) {
            ...
            eachLastestFixDataVersionsModel.set({
                version: eachRow["version"]
            });
            eachLastestFixDataVersionsModel.save();
        } else Ti.API.info("!!Table: " + objectTableName + " UP TO DATE");
    });
});
